"""SOYUZ GAGARIN — engine/data.py v2.1

No-new-account provider strategy.

Primary:
- Twelve Data for XAU/USD (already working in this project).
- Biquote for metals + WTI/Brent. Biquote documents public REST OHLC
  endpoints with 5m/15m/30m/1h intervals and no API key/signup.

Fallback:
- Yahoo for agricultural contracts where Biquote does not expose the
  requested instrument.

Important:
- A quote is LIVE only when the provider timestamp is within the configured
  freshness window.
- Delayed/stale data remains usable for diagnostics/history but can never
  be reported as LIVE.
"""
...
def _parse_biquote(payload):
    ...
    for row in bars:
        # Biquote documents openTime for OHLC bars.
        timestamp = (
            row.get("openTime")
...
def _fetch_biquote(symbol: str):
    ...
    payload, error = _request_json(
        ...
        {
            "interval": "5m",
            "limit": min(max(LOOKBACK, 120), 1000),
        },
        retries=1,
    )
...
def _fetch_yahoo(symbol: str):
    ...
    # One request per host. Do not hammer Yahoo after 429.
    for url_template in YAHOO_URLS:
        ...
        candles, parse_error = _parse_yahoo(
            payload
        )
...
def load_data(
    ...
    # 1. Proven path: Twelve Data for gold.
    if symbol == "XAU/USD":
        candles, error = _fetch_twelve(
            symbol
        )
    ...
    # 2. No-account live/near-live provider
    #    for metals and energy.
    if symbol in BIQUOTE_SYMBOLS:
        candles, error = _fetch_biquote(
            symbol
        )
    ...
    # 3. Agriculture fallback.
    candles, yahoo_error = _fetch_yahoo(
        symbol
    )
