...
def get_data_twelvedata(symbol, interval="1day", outputsize=4000):
    """Twelve Data fetch with lightweight 429 backoff; Yahoo remains primary."""
    if not API_KEY:
    ...
    for attempt, wait_s in enumerate((0, 2, 5), start=1):
        ...
        try:
            response = requests.get(BASE_URL, params=params, timeout=30,
                                    headers={"User-Agent": "CommoditiesBot/3.9"})
            if response.status_code == 429:
                retry_after = response.headers.get("Retry-After")
                try:
                    extra = min(max(int(retry_after), 0), 10) if retry_after else 0
                except Exception:
                    extra = 0
                if extra and attempt < 3:
                    time.sleep(extra)
                last_error = "Twelve Data 429 Too Many Requests"
                continue
            response.raise_for_status()
            data = response.json()
            if data.get("status") == "error":
                msg = data.get("message", "Errore Twelve Data")
                if "rate" in str(msg).lower() or "429" in str(msg):
                    last_error = f"Twelve Data: {msg}"
                    continue
            ...
            raise
    raise RuntimeError(last_error or "Twelve Data non disponibile")
...
def adaptive_risk_levels(analysis, candles, direction):
    ...
    if direction=="LONG":
        ...
        stop_reason = "supporto L2L" if support is not None and abs(stop-support) <= max(a*0.15, price*0.0005) else ("swing low 30 barre" if swing_low is not None and abs(stop-swing_low) <= max(a*0.15, price*0.0005) else "profilo rischio/ATR")
        tp_reasons = ["1° target: 1.35R / profilo ATR" , "2° target: 2.0R / profilo ATR", "3° target: 2.7R / profilo ATR"]
        if resistance is not None and resistance > price:
            tp_reasons[0] += f"; resistenza {resistance:.4f} monitorata"
            tp_reasons[1] += "; resistenza successiva monitorata"
    else:
        ...
        tps=[price-max(tp1_pct*price,1.35*risk), price-max(tp2_pct*price,2.0*risk), price-max(tp3_pct*price,2.7*risk)]
        stop_reason = "resistenza L2L" if resistance is not None and abs(stop-resistance) <= max(a*0.15, price*0.0005) else ("swing high 30 barre" if swing_high is not None and abs(stop-swing_high) <= max(a*0.15, price*0.0005) else "profilo rischio/ATR")
        tp_reasons = ["1° target: 1.35R / profilo ATR", "2° target: 2.0R / profilo ATR", "3° target: 2.7R / profilo ATR"]
        if support is not None and support < price:
            tp_reasons[0] += f"; supporto {support:.4f} monitorato"
            tp_reasons[1] += "; supporto successivo monitorato"
    return {"available": True, "atr": round(a,6), "stop": _price_round(stop), "tp1": _price_round(tps[0]), "tp2": _price_round(tps[1]), "tp3": _price_round(tps[2]),
            "risk_distance": round(risk,6), "method": "STRUCTURA + ATR + PROFILE FALLBACK", "stop_reason": stop_reason,
            "tp_reasons": tp_reasons, "risk_atr": round(risk/max(a,1e-9),2)}
...
def _session_message(label, ranked, best, position_message=None):
    ...
    risk_detail = a.get("risk_detail", {}) or {}
    if risk_detail.get("stop_reason"): lines.append(f"🛑 Perché SL: {risk_detail.get('stop_reason')}")
    tp_reasons = risk_detail.get("tp_reasons", []) or []
    for idx, reason in enumerate(tp_reasons[:3], start=1):
        lines.append(f"📌 Perché TP{idx}: {reason}")
    ...
    for name in COMMODITIES:
        ...
        try:
            ...
            if adaptive_levels.get("available"):
                ...
                analysis["risk_detail"] = adaptive_levels
